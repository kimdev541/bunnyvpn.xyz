...
function is_root() {
  if [[ 0 == "$UID" ]]; then
    print_ok "The current user is the root user, starting the installation process"
  else
...
function system_check() {
  ...
  if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
    ...
    # nginx installation preprocessing
    ...
    print_error "Current system ${ID} ${VERSION_ID} is not in the list of supported systems"
    exit 1
  fi
...
function dependency_install() {
  ...
  # Nginx post-installation no need to compile no longer needed
  #  if [[ "${ID}" == "centos" ||  "${ID}" == "ol" ]]; then
